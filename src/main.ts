...
import TerrariumClient from "./lib/Client";

export { TerrariumServer, TerrariumClient };
...
if (require.main === module)
	(async function () {
		...
		let [destinationAddress, destinationPort] = args[0] ? args[0].split(":"): ["127.0.0.1", "7777"];
		...
		server.on("connection", (client) => {
			...
			client.on("close", () => {
				console.log(
					`${client.remoteAddress}${client.remotePort} disconnected.`
				);
			});
		});
	})();
